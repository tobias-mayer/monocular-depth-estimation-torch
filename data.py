...
def get_train_test_dataloader(batch_size):
    train_dataset = MemoryDataset(
        csv_path='./dataset/nyu2_train.csv', img_dir='./dataset', transform=train_transform())
    test_dataset = MemoryDataset(
        csv_path='./dataset/nyu2_test.csv', img_dir='./dataset', transform=test_transform())

    return (
        DataLoader(train_dataset, batch_size, shuffle=True, num_workers=4),
        DataLoader(test_dataset, batch_size, shuffle=False, num_workers=4)
    )
